...
Read `.pi/skills/greptile-triage.md` if it exists and follow the fetch, filter, classify, and **escalation detection** steps. If the file does not exist, use your judgment to classify Greptile comments as VALID & ACTIONABLE, VALID BUT ALREADY FIXED, or FALSE POSITIVE based on the diff evidence.
...
2. **Read `.pi/skills/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
4. **Apply the design checklist** against the changed files. For each item:
   ...
   - **[LOW] intent-based detection**: present as "Possible — verify visually or run the design review skill"
...
1. **VALID & ACTIONABLE comments:** These are included in your findings — they follow the Fix-First flow (auto-fixed if mechanical, batched into ASK if not) (A: Fix it now, B: Acknowledge, C: False positive). If the user chooses A (fix), reply with inline diff + explanation showing what was fixed. If the user chooses C (false positive), reply with evidence explaining why it is a false positive, save to both per-project and global greptile-history.
...
2. **FALSE POSITIVE comments:** Present each one by asking the user:
   ...
   If the user chooses A, reply with evidence explaining why it is a false positive, save to both per-project and global greptile-history.

3. **VALID BUT ALREADY FIXED comments:** Reply explaining what was already fixed — no ask the user needed:
...
If A: address the findings. Re-run the review to verify.
