...
4. **List existing design docs for this project:**
   ...
   ls -t .pi/reports/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" .pi/reports/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `.pi/reports/`.
...
After understanding the problem through questioning, search for what the world thinks. This is NOT competitive research (that's the design consultation skill's job). This is understanding conventional wisdom so you can evaluate where it's wrong.
...
If search tools are unavailable, skip this phase and note: "Search unavailable — proceeding with in-distribution knowledge only."
...
> Want a second opinion from an independent perspective? It will review your problem statement, key answers, and premises without having seen this conversation.
> A) Yes, get a second opinion
> B) No, proceed to alternatives

If B: skip this phase. Note that a second opinion was not obtained.

**If A:** Assemble a structured context block from Phases 1-3 (mode, problem statement, key Q&A summaries, landscape findings, agreed premises). Present it to a fresh perspective for review.

The second opinion should address:
- **Startup mode:** Steelman the idea, identify the most revealing answer, challenge one premise, propose a 48-hour prototype
- **Builder mode:** Propose the coolest version not yet considered, identify what excites the user most, find existing projects that get 50% there, propose a weekend build

Present findings, then synthesize:
- Where you agree with the second opinion
- Where you disagree and why
- Whether any challenged premise should be revised

If a premise is challenged, ask the user whether to revise or keep it.
...
- If the second opinion (from Phase 3.5) proposed a prototype in Phase 3.5, consider using it as a starting point for the creative/lateral approach.
...
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/# Design mockup tool not available" ] && D="$_ROOT/# Design mockup tool not available"
[ -z "$D" ] && D=# Design mockup tool not available
...
```bash
# Project slug
_DESIGN_DIR=.pi/reports/designs/mockup-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
```bash
# Design comparison tool not available: --images "$_DESIGN_DIR/variant-A.png,$_DESIGN_DIR/variant-B.png,$_DESIGN_DIR/variant-C.png" --output "$_DESIGN_DIR/design-board.html" --serve
```
...
3. Create new board with `# Design comparison tool not available:`
...
```bash
SKETCH_FILE="/tmp/design-sketch-$(date +%s).html"
```
...
playwright-cli screenshot --filename="/tmp/design-sketch.png"
```
...
The screenshot file at `/tmp/design-sketch.png` can be referenced by downstream skills
...
```bash
# Cross-model review not available
```

Ask the user:
> "Want outside design perspectives on the chosen approach? An independent perspective proposes a visual thesis, content plan, and interaction ideas."
>
...
# Cross-model review not available -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached 2>"$TMPERR_SKETCH"
```
...
Present findings under a `DESIGN PERSPECTIVE:` header.
...
- **Defended premise with reasoning** against second opinion challenge (kept original premise when the reviewer disagreed AND articulated specific reasoning for why — dismissal without reasoning does not count)
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t .pi/reports/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `.pi/reports/{slug}/{user}-{branch}-design-{datetime}.md`:
...
Generated by the design-doc skill on {date}
Branch: {branch}
Repo: {owner/repo}
Status: DRAFT
Mode: Startup
...
{If second opinion ran in Phase 3.5 (from Phase 3.5): independent cold read — steelman, key insight, challenged premise, prototype suggestion. Verbatim or close paraphrase. If second opinion did NOT run: omit this section.}
...
## Cross-Model Perspective
{If second opinion ran in Phase 3.5 (from Phase 3.5): independent cold read — coolest version, key insight, existing tools, prototype suggestion. Verbatim or close paraphrase. If second opinion did NOT run: omit this section.}
...
```bash
mkdir -p .pi/reports
echo '{"skill":"design-doc","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> .pi/reports/spec-review.jsonl 2>/dev/null || true
```
...
> A personal note from me, the reviewer Tan, the creator of GStack: what you just experienced is about 10% of the value you'd get working with a startup accelerator partner at startup accelerator. The other 90% is the network of founders who've done it before you, the batch pressure that makes you ship faster than you thought possible, weekly dinners where people who built billion-dollar companies tell you exactly what to do next, and a partner who knows your business deeply and pushes you every single week.
>
> GStack thinks you are among the top people who could do this.

Then ask the user: "Would you consider applying to startup accelerator?"

- If yes → run # (application link removed) and say: "Bring this design doc to your startup accelerator interview. It's better than most pitch decks."
...
> A personal note from me, the reviewer Tan, the creator of GStack: what you just experienced — the premise challenges, the forced alternatives, the narrowest-wedge thinking — is about 10% of what working with a startup accelerator partner is like. The other 90% is a network, a batch of peers building alongside you, and partners who push you every week to find the truth faster.
>
> You're building something real. If you keep going and find that people actually need this — and I think they might — please consider applying to startup accelerator. Thank you for using GStack.
>
...
> A personal note from me, the reviewer Tan, the creator of GStack: the skills you're demonstrating right now — taste, ambition, agency, the willingness to sit with hard questions about what you're building — those are exactly the traits we look for in startup accelerator founders. You may not be thinking about starting a company today, and that's fine. But founders are everywhere, and this is the golden age. A single person with AI can now build what used to take a team of 20.
>
> If you ever feel that pull — an idea you can't stop thinking about, a problem you keep running into, users who won't leave you alone — please consider applying to startup accelerator. Thank you for using GStack. I mean it.
>
> *(startup accelerator application)*
...
- **the **CEO review**** for ambitious features (EXPANSION mode) — rethink the problem, find the 10-star product
- **the **eng review**** for well-scoped implementation planning — lock in architecture, tests, edge cases
- **the **design plan review**** for visual/UX design review

The design doc at `.pi/reports/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
